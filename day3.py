...
# directions = [["R75","D30","R83","U83","L12","D49","R71","U7","L72"], ["U62","R66","U55","R34","D71","R55","D58","R83"]]
# directions = [["R98","U47","R26","D63","R33","U87","L62","D20","R33","U53","R51"], ["U98","R91","D20","R16","D67","R40","U7","R15","U6","R7"]]

class segment(object):
    def __init__(self, start_point, cable_distance, desc):
        self.x, self.y = start_point
        self.cable_distance = cable_distance
        self.direction = desc[0]
        self.distance = int(desc[1:])
        self.is_horisontal = self.direction in ["L", "R"]
        if self.direction == "U":
            self.min_coord = self.y
            self.max_coord = self.y + self.distance
        elif self.direction == "R":
            self.min_coord = self.x
            self.max_coord = self.x + self.distance
        elif self.direction == "D":
            self.max_coord = self.y
            self.min_coord = self.y - self.distance
        else:
            self.max_coord = self.x
            self.min_coord = self.x - self.distance

    def passes(self, value):
        return value > self.min_coord and value < self.max_coord

    def get_intersect(self, other):
        if self.is_horisontal == other.is_horisontal:
            # Assuming parallel lines can't intersect
            return None
        ...
        if vertical.passes(horisontal.y) and horisontal.passes(vertical.x):
            vertical_length = vertical.cable_distance + abs(vertical.y - horisontal.y)
            horisontal_length = horisontal.cable_distance + abs(vertical.x - horisontal.x)
            return vertical_length + horisontal_length
        else:
            return None
...
def main():
    ...
    for i in range(2):
        ...
        for direction in directions[i]:
            current_segment = segment(point, cable_length, direction)
            point = current_segment.get_next_point(point)
            cable_length += current_segment.distance
            cables[i].append(current_segment)

    closest_distance = math.inf
    for segment0 in cables[0]:
        for segment1 in cables[1]:
            intersect_distance = segment0.get_intersect(segment1)
            if intersect_distance:
                closest_distance = min(intersect_distance, closest_distance)
